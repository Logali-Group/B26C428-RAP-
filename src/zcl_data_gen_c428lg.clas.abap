CLASS zcl_data_gen_c428lg DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_data_gen_c428lg IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.

    out->write( 'Deleting existing data...' ).

    DELETE FROM ztravel_428lg.

    INSERT ztravel_428lg FROM (
    ...
    DELETE FROM zbooking_428lg.

    INSERT zbooking_428lg FROM (

      SELECT
        FROM /dmo/booking
        JOIN ztravel_428lg ON /dmo/booking~travel_id = ztravel_428lg~travel_id
        JOIN /dmo/travel ON /dmo/travel~travel_id = /dmo/booking~travel_id
        FIELDS
          "client,
          uuid( ) AS booking_uuid,
          ztravel_428lg~travel_uuid AS parent_uuid,
          ...
          ztravel_428lg~last_changed_at AS local_last_changed_at ).

    out->write( 'Adding Booking Supplements data' ).

    DELETE FROM zbsuppl_428lg.

    INSERT zbsuppl_428lg FROM (
      SELECT FROM /dmo/book_suppl AS supp
        JOIN ztravel_428lg  AS trvl ON trvl~travel_id = supp~travel_id
        JOIN zbooking_428lg AS book ON book~parent_uuid = trvl~travel_uuid
